...
for i = 1:length(zoomfactor)
    ...
    EJO = (- 2*16 * J_M(3,:) * TC./Ww)';                   % g O2/d, O2 consumption (Fenneropenaeus chinensis)

    % Excretion (Augusto 2016 - mg e Augusto 2014 - mg)
    EJN = (1000 * J_M(4,:) * TC * 17.031 ./ Ww)';                    % mg-at NH3/d, ammonia production
...
%Results Wg
Wg = zeros(50,5);
load ('Results_0.10152.mat')
Wg (:,1) = results.Wg;
load ('Results_0.10767.mat')
Wg (:,2) = results.Wg;
load ('Results_0.065596.mat')
Wg (:,3) = results.Wg;
load ('Results_0.075274.mat')
Wg (:,4) = results.Wg;
load ('Results_0.095441.mat')
Wg (:,5) = results.Wg;
csvwrite ( 'Wg.csv' , Wg)
